<?php 
    ...
    $total_unread = "SELECT COUNT(id) as total FROM tbl_contacts WHERE is_seen = false AND is_important=false";
    $inbox = $db->select($total_unread)->fetch_object();
...
<div class="email-left-box"><a href="mail-compose.php" class="btn btn-primary btn-block">Compose</a>
    <div class="mail-list mt-4">
        ...
        <a href="#" class="list-group-item border-0 p-r-0"><i class="fa fa-trash font-18 align-middle mr-2"></i>Trash
            <?php if($inbox->total > 0){ ?>
                <span class="badge badge-danger badge-sm float-right m-t-5"><?php echo $inbox->total; ?></span>
            <?php } ?>
        </a>
    </div>
